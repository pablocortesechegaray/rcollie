...
- Mediante parametrización estándard.
- Mediante un desarrollo JavaScript que aplique un filtro personalizado sobre el lookup.

Personalmente recomiendo que se haga uso del prefiltrado mediante parametrización estándar siempre y cuando sea posible, aunque la realidad es que en ocasiones por necesidades del proyecto o de la lógica de negocio del cliente con el que estamos trabajando nos vemos obligados a aplicar filtros dinámicos para lo cual necesitamos realizar uso del método addPreSearch() de JavaScript.
...
function FiltrarLookup(formContext) {
    ...
    const lookupControl = formContext.getControl("productid");
    ...
    if (lookupControl != null) {
        ...
        lookupControl.addPreSearch(() => {
            lookupControl.addCustomFilter(filtroXML, "product");
        });
    }
}
```